			...
			<scroll-view scroll-x="true" scroll-with-animation class="scroll-tab width center">
				<view class="card">
					<navigator url="../glucose/glucose" hover-class="none">
						...
						<view class="data">
							{{avgGlucoseLevelAverage.length==0 ? "0":avgGlucoseLevelAverage[avgGlucoseLevelAverage.length-1].avgGlucoseLevel}}mmol/L
						</view>
						<view class="bottom">
	...
	export default {
		...
		methods: {
			...
			showNav() {
				...
				console.log(this.avgGlucoseLevelAverage)
			},
			closeNav() {
				this.navShow = false
				var scale = 0.7
			...
			async getAveGlucoseLevelAge(){
				let result=await this.$store.dispatch('getHeartHistory')
			}
		},
		computed:{
			...mapState({
